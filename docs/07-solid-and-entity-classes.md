...
**Chapter 08 — [Game States & the Standard Controller](08-game-states.md).**
Now that entities sit behind a single interface, adding things that move and
draw is cheap. Next: give the game a skeleton — a `GameState` finite state
machine with MENU, PLAYING, and GAME_OVER screens, plus a standard input
layer (arrows move, X confirms, C cancels). After that: sound.
